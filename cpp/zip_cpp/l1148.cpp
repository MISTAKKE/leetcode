...
public:
    vector<int> avoidFlood(vector<int> &rains)
    {
        ...
        map<int, bool> source; //代表了当前水库是否是空的  source[k]=true 水库k是满的   否则就是空的
        int idx = 0;
        for (int i = 0; i < rains.size(); ++i)
        {
            if (rains[i] == 0) //当天晴天，准备排水
            {
                ...
                {
                    if (source.find(rains[idx]) != source.end() && source[rains[idx]] == true)
                    {
            ...
            else //当天雨天，准备放工
            {
                ...
                if ((source.find(rains[i]) != source.end()) && (source[rains[i]] == true))
                {
