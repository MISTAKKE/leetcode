...
public:
    ...
    TreeNode *helper(vector<int> &inorder, vector<int> &postorder, int inorder_left, int inorder_right, int postorder_left, int postorder_right)
    {
        ...
        root->left = helper(inorder, postorder, inorder_left, inorder_left + left_subtree_size - 1, postorder_left, postorder_left + left_subtree_size - 1);

        //right
        root->right = helper(inorder, postorder, inorder_root_idx + 1, inorder_right, postorder_left + left_subtree_size, postorder_right - 1);
        return root;
    }
};
