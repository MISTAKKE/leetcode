...
public:
    bool isValidBST(TreeNode *root, long minval = __LONG_MAX__ * -1, long maxval = __LONG_MAX__)
    {
        ...
        if (root->left != NULL)
        {
            if (root->left->val >= root->val || root->left->val >= maxval || root->left->val <= minval || !isValidBST(root->left, minval, root->val))
            {
                return false;
            }
        }
        if (root->right != NULL)
        {
            if (root->val >= root->right->val || root->right->val >= maxval || root->right->val <= minval || !isValidBST(root->right, root->val, maxval))
            {
                return false;
            }
        }
