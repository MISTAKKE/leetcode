...
public:
    ...
    {
        if (fathermp.find(a) == fathermp.end() || fathermp.find(b) == fathermp.end() || find(fathermp, a) != find(fathermp, b))
        {
            return -1;
        }
...
int main()
{
    ...
    // vector<double> values{2, 3, 4, 5, 6, 7, 8};
    // vector<vector<string>> queries{{"a","z"}, {"b","c"}, {"a","e"}, {"a","a"}, {"x","x"}, {"a","z"}};
    //show(A.calcEquation(equations, values, queries));
